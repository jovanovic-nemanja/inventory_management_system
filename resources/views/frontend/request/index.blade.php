  ...
  <div class="datatablestructure">
                ...
                @if($user_status == 0)
                  @if($request['status'] == 1)
                    <a class="btn btn-success"
                      ...
                      id="{{ $request['id'] }}" style="color: #fff;">View</a>
                    <a class="btn btn-success"
                      href="{{ route('request.change', $request['id']) }}"
                      id="{{ $request['id'] }}">Edit</a>
                  ...
                  @if($request['status'] == 1)
                    <a class="btn btn-danger delete"
                      href="{{ url('/request/destroy', $request->id) }}"
    ...
    @if(auth()->user()->hasRole('seller'))
      ...
      <table id="example" class="table dt-responsive" cellspacing="0" width="100%">
        ...
        <thead>
          <tr>
            <!--<th class="sorting_desc" >Request ID</th>-->
            <th>Request ID</th>
            <th>Sent Date</th>
            ...
            <tr>
              ...
              @if($user_status == 0)
                <td style="white-space: nowrap">
                  ...
                  <a class="btn btn-success detail btn-small"
                    href="{{ route('request.view', $request->id) }}"
                    id="{{ $request->id }}">Detail</a>
                  @if($request->is_quotes($request->id) == -1)
                    <a class="btn btn-success btn-small"
                      href="{{ url('/quote/reply', $request->id) }}"
                      id="{{ $request->id }}" style="color: #fff;">Quote Now</a>
                  @elseif($request->is_quotes_accept($request->id) == 1)
                    <a class="btn btn-success btn-small"
                      href="{{ url('/quote/edit', $request->id) }}"
                      id="{{ $request->id }}" style="color: #fff;">Quote Edit</a>
                  @else
                    Quote submitted
                  @endif
